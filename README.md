...
- Shape recognition: hold the pointer still for 250 ms at the end of a line or closed shape. The live stroke snaps before lift and tolerates small Pencil movement afterward. Equal-distance resampling and normalized template matching recognize forgiving lines, rectangles, perfect circles, and common single-stroke arrows; near-horizontal and near-vertical lines keep their starting point fixed. Undo removes the recognized result.
- Eraser modes: whole-stroke erasing is enabled by default. Toggle the eraser-mode button for precise partial erasing; precise mode previews only the area inside the eraser disk with the same opacity as whole-stroke mode, and both modes support undo.
