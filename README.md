...
- Mobile layout: contextual toggles stay on the left and appear only for their tool (scale lock for Select, shape recognition for Draw, eraser mode for Erase). Select/Draw/Erase stay centered, and colours move to a visible second row before controls overlap on narrow iPad or phone canvases.
