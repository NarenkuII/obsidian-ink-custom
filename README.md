...
- Selection: drag from inside the frame, large touch targets, uniform corner scaling, rotation, and finger support. The selected stroke frame and its handles keep pointer priority over an image underneath. A recognized line uses two endpoint handles plus a central move handle instead of a large box.
